defmodule BittorrentClient.TCPConn.InMemoryImpl do
  ...
  def connect({0, 0, 0, 0}, _port, _opts) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.connect/3")
  ...
  def connect({0, 0, 0, 0}, _port, _opts, _timeout) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.connect/4")
  ...
  def accept(tcp_conn) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.accept/1")

    {:ok, %TCPConn{tcp_conn | socket: :mock}}
  end

  def accept(tcp_conn, _timeout) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.accept/2")
  ...
  def listen(_port, _opts) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.listen/2")

    {:ok, %TCPConn{socket: :mock, parent_pid: self()}}
  end

  def recv(_tcp_conn, len) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.recv/2")
  ...
  def recv(_tcp_conn, len, _timeout) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.recv/3")

    {:ok, <<0::size(len)>>}
  end

  def send(_tcp_conn, _packet) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.send/2")

    :ok
  end

  def close(_tcp_conn) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.close/1")

    :ok
  end

  def shutdown(_tcp_conn, _how) do
    Logger.warn("Using #{__MODULE__} implementation of :gen_tcp.shutdown/2")

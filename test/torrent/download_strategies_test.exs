defmodule BittorrentClient.Torrent.DownloadStrategies.Test do
  ...
  test "determine correct piece with rarest strategy", context do
    indexes = [0,1,2,3,4]
    {:ok, index} = DownloadStrategies.determine_next_piece(:rarest_piece, context.piece_table, indexes)
    assert(index == 2)
  ...
  test "determine correct piece with default (in-order) strategy", context do
    indexs = [0,1,2,3,4,5]
    {:ok, index} = DownloadStrategies.determine_next_piece(:in_order, context.piece_table, indexs)
    assert index == 1
  end

  test "handle empty piece table when determining next piece" do
    indexes = [1,2,3,4,5]
    {status, _} = DownloadStrategies.determine_next_piece(:rarest_piece, %{}, indexes)
    assert status == :error
  end

  test "handle empty index list when determining next piece", context do
    {status, _} = DownloadStrategies.determine_next_piece(:rarest_piece, context.piece_table, [])
    assert status == :error
  end
end